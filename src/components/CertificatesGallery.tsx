...
function CertificateDetail({
  ...
  return (
    <Modal label={copy.modalLabel(certificate.title)} onClose={onClose} size="document" closeLabel={copy.close}>
      <article className="certificate-detail">
        <div>
          <img src={certificate.image} alt={copy.imageAlt(certificate.title, certificate.issuer)} decoding="async" />
        </div>
        <footer>
          ...
          {certificate.credential && (
            <a href={certificate.credential} target="_blank" rel="noopener noreferrer">
              {copy.validate} <ExternalLink />
            </a>
          )}
        </footer>
      </article>
...
export function CertificatesGallery({
  ...
  return (
    <>
      <Section id={sectionId} className="certificates-section">
            ...
            {certificates.map((certificate) => (
              <article className="certificate-card" key={certificate.title}>
                <button type="button" onClick={() => setSelected(certificate)} aria-label={`${copy.enlarge} ${certificate.title}`}>
                  <span><img src={certificate.image} alt="" loading="lazy" decoding="async" /></span>
                  <i>{copy.enlarge}</i>
                </button>
                <div>
                  ...
                  {certificate.credential && <a href={certificate.credential} target="_blank" rel="noopener noreferrer">{copy.validate} <ExternalLink /></a>}
                </div>
              </article>
            ))}
          </div>
          ...
          <div className="events-grid">
            {events.map((event) => (
              <article key={event.title}>
                <a className="event-card__preview" href={event.file} target="_blank" rel="noopener noreferrer" aria-label={copy.eventViewAria(event.title)}>
                  <img src={event.image} alt={copy.eventPreviewAlt(event.title)} loading="lazy" decoding="async" />
                  <i>{copy.view}</i>
                </a>
                <div className="event-card__content">
                  ...
                  <h4>{event.title}</h4>
                  <p>{event.detail}</p>
                  <a href={event.file} target="_blank" rel="noopener noreferrer">{copy.viewCertificate} <ExternalLink /></a>
                </div>
              </article>
            ))}
          </div>
